...
export default function ListsScreen({ navigation }) {
  ...
  const handleDeleteList = (listName) => {
    fetch(
      `${process.env.EXPO_PUBLIC_BACKEND_URL}/lists/${listName}/${user.username}`,
      {
        method: "DELETE",
  ...
  const handleSeeList = (listName) => {
    ...
    fetch(
      `${process.env.EXPO_PUBLIC_BACKEND_URL}/lists/getGames/${listName}/${user.username}`
    )
      .then((response) => response.json())
      .then((data) => {
        let gamesList;
          ...
          gamesList = data.list.map((data, i) => {
            ...
            return (
              <View key={i} style={styles.game}>
                ...
                <TouchableOpacity
                  onPress={() => handleDeleteGame(listName, data[0])}
                  style={styles.button}
                >
  ...
  const handleDeleteGame = (listName, gameName) => {
    fetch(
      `${process.env.EXPO_PUBLIC_BACKEND_URL}/games/${listName}/${gameName}/${user.username}`,
      {
        method: "DELETE",
      }
    )
    ...
    let trash_bin =
      i < 1 ? (
        ...
        <FontAwesome
          ...
          color="#ffffff"
          size={16}
          onPress={() => handleDeleteList(data.listName)}
        />
      );
